...
describe("parseGPX", () => {
  it("parses a simple track with multiple points", async () => {
    const gpx = makeGPX(
  ...
  it("detects gaps when time between points exceeds 60s", async () => {
    const gpx = makeGPX(
  ...
  it("returns empty arrays for empty GPX", async () => {
    const gpx = makeGPX("");
    const result = await parseGPX(gpx);
  ...
  it("handles a single trackpoint", async () => {
  ...
  it("throws on malformed XML", async () => {
    await expect(parseGPX("<gpx><not-closed>")).rejects.toThrow("Invalid GPX");
  });

  it("handles multiple trkseg elements across tracks", async () => {
...
<gpx xmlns="http://www.topografix.com/GPX/1/1" version="1.1">
  ...
  it("skips points without valid time", async () => {
    const gpx = makeGPX(
  ...
  it("handles points without elevation", async () => {
    const gpx = makeGPX(pt(37.6, -1.03, "2026-06-06T05:00:00Z"));
    const result = await parseGPX(gpx);
    expect(result.trackpoints[0].ele).toBeUndefined();
  });

  it("boundary: exactly 60s is NOT a gap", async () => {
    const gpx = makeGPX(
      pt(37.6, -1.03, "2026-06-06T05:00:00Z") +
  ...
  it("boundary: 61s IS a gap", async () => {
    const gpx = makeGPX(
      pt(37.6, -1.03, "2026-06-06T05:00:00Z") +
